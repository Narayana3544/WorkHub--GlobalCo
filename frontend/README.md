...
1. **Log in** with your email and password (no OTP step — kept deliberately frictionless for v1; see PRD roadmap for when this changes). There is no public self-service registration — see "Account Creation" below.
2. You land on the **Home launcher**, not directly on a module. This is a role-aware tile grid showing only the modules you actually have access to:
   - **Employee:** Task Manager, My Profile & Documents, Leave Management, Performance Reviews — each scoped to your own work.
   - **Manager:** the same 4 tiles, plus **Team Approvals** (leave/timesheet approvals for their reports).
   - **Admin:** all of the above, plus **Administration** (RBAC/permissions, master data, user creation).
   - A tile you can't use is never shown — not grayed out, simply absent — so there's no ambiguity about what's off-limits.
3. Each tile shows a live count where relevant (e.g. pending approvals, tasks due today), pulled from real data, never a placeholder.
4. Click a tile to enter that module (**Task Manager**, **Profile & Documents**, **Leave**, **Performance**, or **Administration**). The top nav (logo, your name + role badge, logout) is present on every screen — click the logo to return to the Home launcher at any time.
5. **Logging out** clears both tokens and returns you to `/login`. Any route accessed without a valid session redirects to `/login` automatically — there is no way to view a module screen unauthenticated.

### Account Creation

WorkHub does not have a public Register page. This follows the PRD's stated rule that users are created by HR or Admin, not by open signup — appropriate for an internal MSME tool, not a public-facing product. New users are created from the **Administration** tile (name, email, temporary password, role, org). Since there's no OTP/email flow to deliver credentials in v1, the temporary password is shown once at creation time for the Admin to share directly with the new user.
